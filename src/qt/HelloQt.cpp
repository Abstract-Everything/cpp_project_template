...
HelloQt::HelloQt(QWidget *parent) : QWidget(parent)
{
	...
	// The tr calls are not strictly neccesary, but it is all that is needed
	// for future i18n.
	...
	// Signals and slots are use to simply define relationships between
	// actions and effects By using this "new style" connect the types are
	// checked at compile time to match
	...
	connect(fatal, &QPushButton::clicked, this, &HelloQt::close); // could
								      // just
								      // use
								      // close
};
